...
namespace Rowlan.Yapp
{
    public class BrushDistribution
    {
        ...
        public void CreatePreviewPrefab()
        {
            ...
            if (previewPrefab.prefabSettings == null)
                return;
        ...
        {
            if (CanAddPrefabAt(position, normal))
            {
        ...
        /// <summary>
        /// Check if the preview prefab can be added at the given position.
        /// Will be used for the brush color to indicate whether adding is allowed or not.
        /// </summary>
        /// <param name="position"></param>
        /// <param name="normal"></param>
        /// <returns></returns>
        public bool CanAddPrefabAt(Vector3 position, Vector3 normal)
        {
            ...
            if (!editorTarget.brushSettings.allowOverlap)
            {
                foreach (Transform child in container.transform)
                {
                    ...
                    if (dist <= (editorTarget.brushSettings.brushSize + editorTarget.brushSettings.overlapMargin))
                    {
                        prefabExists = true;
                        break;
                    }
        ...
        private PrefabTransform CreateAppliedTransform(PrefabSettings prefabSettings, Vector3 position, Vector3 normal)
        {
            ...
            if (this.editorTarget.brushSettings.alignToTerrain)
            {
                ...
                // slerp between up and normal, create a direction inbetween depending on the slerp value
                Vector3 slerpVector = Vector3.Slerp(Vector3.up, normal, slerpValue);

                // from now on the normal is the slerpVector

                // create perpendicular vector to normal (same as rotation debug gizmo)
                Vector3 perpendicular = Vector3.Cross(slerpVector, Vector3.right);

                // rotate perpendicular vector by brush rotation (same as rotation debug gizmo)
                perpendicular = Quaternion.AngleAxis(editorTarget.brushSettings.brushRotation, slerpVector) * perpendicular;

                // create final rotation that matches the rotation debug gizmo
                newRotation = Quaternion.LookRotation(perpendicular, slerpVector);

                // apply object rotation (eg random rotation)
                newRotation *= objectRotation;
            }
            else
            {
                newRotation =  objectRotation * brushRotation;
            }
        ...
        /// </summary>
        public void AddPrefabs_Poisson_Any(Vector3 position, Vector3 normal, bool forceNewDistribution, int density, float overlapOffset)
        {
            GameObject container = editorTarget.container as GameObject;
            ...
            int discDensity = editorTarget.brushSettings.poissonDiscDensity;

            List<Vector2> samples = PoissonDiscSampleProvider.Instance.Samples(brushSize, brushSize, discRadius, forceNewDistribution);

            // density: remove random discs
            if (samples.Count > 0 && density < 100)
            {
                List<Vector2> densityList = new List<Vector2>(samples);

                float densityMax = samples.Count * density / 100f;
                
                while (densityList.Count > densityMax)
                {
                    int randomIndex = Random.Range(0, densityList.Count - 1);
                    densityList.RemoveAt(randomIndex);
                }
            ...
            foreach (Vector2 sample in samples)
            {
                ...
                if (!editorTarget.brushSettings.allowOverlap)
                {
                    foreach (Transform child in container.transform)
                    {
                        ...
                        if (dist <= (discSize + overlapOffset))
                        {
                            prefabExists = true;
                            break;
                        }
        ...
        /// Add prefabs, mode Poisson
        /// </summary>
        public void AddPrefabs_Poisson_Terrain(Vector3 position, Vector3 normal, bool forceNewDistribution, int density, float overlapOffset)
        {
