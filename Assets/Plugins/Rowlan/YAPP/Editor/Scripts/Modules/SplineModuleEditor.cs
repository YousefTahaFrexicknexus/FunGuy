...
namespace Rowlan.Yapp
{
    public class SplineModuleEditor: ModuleEditorI
    {
        ...
        SerializedProperty lanePosition;

        SerializedProperty instanceRotation;
        SerializedProperty instanceRotationX;
        SerializedProperty instanceRotationY;
        SerializedProperty instanceRotationZ;

        SerializedProperty controlPointRotation;
        SerializedProperty attachMode;
        SerializedProperty prefabSelection;
        ...
        public SplineModuleEditor(PrefabPainterEditor editor)
        {
            ...
            lanePosition = editor.FindProperty(x => x.splineSettings.lanePosition);

            instanceRotation = editor.FindProperty(x => x.splineSettings.instanceRotation);
            instanceRotationX = editor.FindProperty(x => x.splineSettings.instanceRotationX);
            instanceRotationY = editor.FindProperty(x => x.splineSettings.instanceRotationY);
            instanceRotationZ = editor.FindProperty(x => x.splineSettings.instanceRotationZ);
            ...
            prefabSelection = editor.FindProperty(x => x.splineSettings.prefabSelection);
        ...
        public void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(lanePosition, new GUIContent("Lane Position", "Set lane position relative to spline (i.e. prefab distribution) to both (left and right) or only left or only right"));
            ...
            {
                // note: indenting here wasn't possible, the labels of the horizontal toggles disappeared
                //       so solution is to indent all on their own
                //EditorGUI.indentLevel++;
                {
                    EditorGUI.indentLevel++;
                    EditorGUILayout.PropertyField(controlPointRotation, new GUIContent("Control Point Rotation", "Enable rotation of the spline control points"));
                    EditorGUI.indentLevel--;

                    EditorGUILayout.BeginHorizontal();
                    {
                        EditorGUI.indentLevel++;
                        EditorGUILayout.PrefixLabel( new GUIContent( "Allowed Rotation", "Prefabs are rotated along the spline, with this you can disable specific directions"));
                        EditorGUI.indentLevel--;

                        int labelWidth = 14;
                        int controlWidth = 20;
                        EditorGUILayout.LabelField("X", GUILayout.Width(labelWidth), GUILayout.ExpandWidth(false));
                        EditorGUILayout.PropertyField(instanceRotationX, GUIContent.none, GUILayout.Width(controlWidth), GUILayout.ExpandWidth(false));

                        EditorGUILayout.LabelField("Y", GUILayout.Width(labelWidth), GUILayout.ExpandWidth(false));
                        EditorGUILayout.PropertyField(instanceRotationY, GUIContent.none, GUILayout.Width(controlWidth), GUILayout.ExpandWidth(false));

                        EditorGUILayout.LabelField("Z", GUILayout.Width(labelWidth), GUILayout.ExpandWidth(false));
                        EditorGUILayout.PropertyField(instanceRotationZ, GUIContent.none, GUILayout.Width(controlWidth), GUILayout.ExpandWidth(false));
            ...
            EditorGUILayout.PropertyField(prefabSelection, new GUIContent("Prefab Selection", "How prefabs are picked from the prefab pool for placement."));

            EditorGUILayout.PropertyField(reusePrefabs, new GUIContent("Reuse Prefabs", "If active, then already created prefabs will be reused. Otherwise new prefabs are created whenever something changes on the spline."));
        ...
        public void OnSceneGUI()
        {
            ...
            string info = $"Add Control Point: shift + click\nRemove control point: shift + ctrl + click\nChange Attach Mode: shift + A, Current: {editorTarget.splineSettings.attachMode}";
        ...
        */
        ...
        private void ClearSpline( bool removePrefabInstances)
        {
            ...
            editorTarget.splineSettings.prefabInstanceScaleMap.Clear();
            editorTarget.splineSettings.prefabInstanceRotationMap.Clear();
