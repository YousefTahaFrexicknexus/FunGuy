...
namespace Rowlan.Yapp
{
    public class BrushComponent
    {
        ...
        /// <summary>
        /// Draw the brush using the colors depending on the current mode
        /// </summary>
        /// <param name="mode"></param>
        /// <param name="brushSettings"></param>
        /// <param name="brushDistribution">The distribution object used for evaluation whether adding prefabs is allowed or not. Can be null. In that case painting will be allowed without check. Note that this is different to the overlap check</param>
        /// <param name="brushMode"></param>
        /// <param name="mouseHit"></param>
        /// <returns></returns>
        public bool DrawBrush( PrefabPainter.Mode mode, BrushSettings brushSettings, BrushDistribution brushDistribution, out BrushMode brushMode, out RaycastHit mouseHit)
        {
            ...
            if (Physics.Raycast(ray.origin, ray.direction, out RaycastHit hit, Mathf.Infinity, layerMask))
            {
                ...
                if (Event.current.control)
                {
                    ...
                    {
                        // get x or y for scroll delta
                        // older unity versions use y; however in later versions (eg 2022.3.16f1) unity introduced that shift changes the variable
                        // so that the shift key swaps the values
                        float delta = 0;
                        if (Event.current.delta.x != 0 || Event.current.delta.y != 0)
                        {
                            delta = Event.current.delta.y == 0 ? Event.current.delta.x : Event.current.delta.y;
                        }

                        // ctrl + shift + scroll = brush rotation
                        if (Event.current.shift)
                        {
                            int rotationStepSize = brushSettings.fluentRotationStepDegrees;
                ...
                {
                    // adding depends on the mode, eg in physics paint mode adding isn't considered
                    // this evaluates in the brush paint mode whether adding a prefab at the brush position is allowed or not
                    bool addPositionValid = brushDistribution == null ? true : brushDistribution.CanAddPrefabAt(hit.point, hit.normal);

                    DrawBrush(mode, brushSettings, hit.point, hit.normal, radius, brushSettings.overlapMargin, brushMode, addPositionValid);
                }
        ...
        private void DrawBrush(PrefabPainter.Mode mode, BrushSettings brushSettings, Vector3 position, Vector3 normal, float radius, float overlapMargin, BrushMode brushMode, bool addPositionValid)
        {
            // set default colors
            ...
            switch (brushMode)
            {
                ...
                case BrushMode.ShiftDrag:
                    innerColor = addPositionValid ? GUIStyles.BrushAddInnerColor : GUIStyles.BrushAddInvalidInnerColor;
                    outerColor = addPositionValid ? GUIStyles.BrushAddOuterColor : GUIStyles.BrushAddInvalidOuterColor;
                    break;
            ...
            switch (mode)
            {
                case PrefabPainter.Mode.Brush:
                    DrawPaintBrush(brushSettings, position, normal, radius, overlapMargin, innerColor, outerColor, brushMode);
                    break;
        ...
        private void DrawPaintBrush(BrushSettings brushSettings, Vector3 position, Vector3 normal, float radius, float overlapMargin, Color innerColor, Color outerColor, BrushMode brushMode)
        {
            ...
            switch (brushSettings.distribution)
            {
                ...
                case BrushSettings.Distribution.Poisson_Terrain:
                    ...
                    if (brushSettings.sizeGuide)
                    {
                        ...
                        // calculate the radius including maring for the fill area. margin can be negative or positive, depending on the use case
                        float totalRadius = Mathf.Max(radius, radius + overlapMargin * 0.5f);
                        Handles.DrawSolidDisc(position, normal, totalRadius);
                        ...
                        if (overlapMargin != 0)
                        {
                            Handles.DrawWireDisc(position, normal, radius + overlapMargin * 0.5f);
                        }
                    }
