...
namespace Rowlan.Yapp
{
    ...
    public class SelectionGrid
    {
        ...
        private static Selectable GetFocused(Selectable[] selectables)
        {
            return selectables.Where(x => x.focused).First();
        }

        /// <summary>
        /// Container class for clipboard information (cut/paste)
        /// </summary>
        public class Clipboard
        {
            public int cutIndex = -1;
            public int pasteIndex = -1;
            ...
            public bool IsClipboardValid()
            {
                return cutIndex != -1 && pasteIndex != -1;
            }
        }

        /// <summary>
        /// Check the clipboard specific keyboard shortcuts and register data (focused selectable, cut index, paste index)
        /// </summary>
        /// <param name="clipboard"></param>
        /// <param name="selectables"></param>
        /// <returns></returns>
        private static bool ProcessClipboardShortcuts(Clipboard clipboard, Selectable[] selectables)
        {
            // cut
            if (Event.current.type == EventType.KeyDown)
            {
                ...
                {
                    Selectable focused = GetFocused(selectables);
                    int index = selectables.ToList().IndexOf(focused);

                    // invalidate potential paste index
                    clipboard.Reset();

                    // register new cut index
                    clipboard.cutIndex = index;                    
            ...
            if (Event.current.type == EventType.KeyDown)
            {
                if (Event.current.keyCode == KeyCode.V)
                {
                    ...
                    // register paste index
                    clipboard.pasteIndex = index;
        ...
        /// <returns></returns>
        public static bool ShowSelectionGrid(List<int> selectedIndexes, Selectable[] selectables, int cellSize, Clipboard clipboard, bool dynamicResize = false, string title = null, bool missingDataTextVisible = false)
        {
            // handle movement of preset items via keyport shortcut
            bool clipboardChanged = ProcessClipboardShortcuts(clipboard, selectables);
            if (clipboardChanged)
                return true;
            ...
            {
                EditorGUILayout.HelpBox("Selection: Single = Click, Add = Control+Click, Range = Shift+Click", MessageType.None);
                EditorGUILayout.HelpBox("Clipboard: Cut = Ctrl+X, Paste = Ctrl+V", MessageType.None);
            }
            ...
            {
                List<Selectable> selectedItems = GetSelectedItems(selectedIndexes, selectables);
                
                foreach (Selectable selectable in selectables)
                    selectable.active = selectedItems.Contains(selectable);
        ...
        /// <summary>
        /// Get the a list of selected selectables
        /// </summary>
        /// <param name="selectedIndexes"></param>
        /// <param name="selectables"></param>
        /// <returns></returns>
        public static List<Selectable> GetSelectedItems(List<int> selectedIndexes, Selectable[] selectables)
        {
            ...
            foreach (Selectable selectable in selectables)
            {
                ...
                bool selected = index < selectables.Length && selectedIndexes.Contains(index);
