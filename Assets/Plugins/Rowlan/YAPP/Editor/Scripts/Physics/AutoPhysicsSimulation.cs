...
namespace Rowlan.Yapp
{
    public class AutoPhysicsSimulation
    {
        ...
        public static void ApplyPhysics(PhysicsSettings physicsSettings, Transform[] objects, SpawnSettings.AutoSimulationType autoSimulationType)
        {
            ...
            if (autoSimulationType == SpawnSettings.AutoSimulationType.Enabled)
            {
                physicsSimulation.StartSimulation(objects);

                PhysicsSimulator.SetSimulationFrameFPS((physicsSettings.simulationFrameFPS));
                PhysicsSimulator.SetSimulationTime(physicsSettings.simulationTime);
                PhysicsSimulator.SetSimulationIterations(physicsSettings.simulationSteps);
                PhysicsSimulator.SetSolverIterations(physicsSettings.solverIterations);
                PhysicsSimulator.SetAutoSyncTransforms(physicsSettings.autoSyncTransforms);
