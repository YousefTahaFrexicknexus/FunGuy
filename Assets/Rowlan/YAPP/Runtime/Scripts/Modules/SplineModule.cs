...
namespace Rowlan.Yapp
{
    public class SplineModule
    {
        ...
        private void RemoveAllPrefabInstances()
        {
            ...
            prefabPainter.splineSettings.prefabInstanceScaleMap.Clear();
            prefabPainter.splineSettings.prefabInstanceRotationMap.Clear();
        ...
        private void RemovePrefabInstances( int startIndex)
        {
            ...
            for (int i = prefabPainter.splineSettings.prefabInstances.Count - 1; i >= startIndex; i--)
            {
                ...
                prefabPainter.splineSettings.prefabInstanceScaleMap.Remove( prefab );
                prefabPainter.splineSettings.prefabInstanceRotationMap.Remove(prefab);
        ...
        private GameObject AddPrefab( ref int prefabInstanceIndex, Vector3 position, Vector3 direction, List<SplinePoint> splinePoints, int currentSplinePointIndex)
        {
            ...
            // adjust lane position to left and right / left / right
            switch( prefabPainter.splineSettings.lanePosition)
            {
                case SplineSettings.LanePosition.Both:
                    offsetLane = -prefabPainter.splineSettings.lanes / 2;
                    break;

                case SplineSettings.LanePosition.Left:
                    offsetLane = -prefabPainter.splineSettings.lanes + 1;
                    break;

                case SplineSettings.LanePosition.Right:
                    offsetLane = 0;
                    break;
        ...
        private void ApplyPrefabSettings( int offsetLane, PrefabSettings prefabSettings, GameObject prefab, Vector3 currentPosition, Vector3 direction, List<SplinePoint> splinePoints, int currentSplinePointIndex)
        {
            ...
            switch(prefabPainter.splineSettings.instanceRotation)
            {
                case SplineSettings.Rotation.Spline:
                    ...
                    // consider rotation limits
                    if (prefabPainter.splineSettings.instanceRotationX == false)
                        direction.x = 0f; 
                    
                    if (prefabPainter.splineSettings.instanceRotationY == false)
                        direction.y = 0f;

                    if (prefabPainter.splineSettings.instanceRotationZ == false)
                        direction.z = 0f;
            ...
            // override transform setting if prefabs should be reused
            if (prefabPainter.splineSettings.reusePrefabs)
            {
                // scale
                if( prefabPainter.splineSettings.prefabInstanceScaleMap.TryGetValue(prefab, out Vector3 prefabScale))
                {
                    prefab.transform.localScale = prefabScale;
                }
                else
                {
                    prefabPainter.splineSettings.prefabInstanceScaleMap.Add(prefab, prefab.transform.localScale);
                }

                // rotation
                if (prefabPainter.splineSettings.prefabInstanceRotationMap.TryGetValue(prefab, out Quaternion prefabRotation))
                {
                    prefab.transform.rotation = prefabRotation;
                }
                else
                {
                    prefabPainter.splineSettings.prefabInstanceRotationMap.Add(prefab, rotation);
                }
