...
namespace Rowlan.Yapp
{
    ...
    public class PrefabPainterEditor : BaseEditor<PrefabPainter>
    {
        ...
        public void OnEnable()
        {
            ...
            SceneView.duringSceneGui -= OnSceneGUIPrefabPainter;
            SceneView.duringSceneGui += OnSceneGUIPrefabPainter;
        ...
        private void OnSceneGUIPrefabPainter( SceneView sceneView)
        {
            ...
            // consider dragging the same as mouse movement: consume the event; otherwise eg in Unity 6 while dragging with the left mouse button
            // the cursor appears as if it were lagging
            // it's not really an issue, but got complaints about that, so implementing that "solution" for now
            // skip alt key, that one's in combination is used by unity for camera rotation
            if (consumeDragEvent)
            {
                if (Event.current.isMouse && Event.current.type == EventType.MouseDrag && Event.current.button == 0 && !Event.current.alt)
                    Event.current.Use();
            }
        ...
        public static void ShowGuiInfo(string guiInfo)
        {
            ...
            labelStyle.hover.textColor = textColor; // hovering over text changes color

            // add a little bit of space. some user report problems with cut-off text
            infoRect.y -= 12f;

            /*
             * this was before, but in unity 6 this was blocking. switched to GUI.Label
            ...
            */
            ...
            GUI.Label(infoRect, guiInfo, labelStyle);
        ...
        {
            foreach (PrefabTemplateCollection templateCollection in editor.prefabModule.templateCollections)
            {
                PrefabSettingsTemplate template = templateCollection.templates.Find(x => x.name == templateName);
