...
#pragma warning restore 0414
        ...
        public void PlacePrefab( GameObject prefab, bool autoAddPrototype, Vector3 worldPosition, Vector3 worldScale, Quaternion rotation, float brushSize, bool randomColor, float colorAdjustment, bool allowOverlap, float overlapOffset)
        {
            Terrain terrain = GetTerrain();
            ...
            if (prototypeIndex == -1)
            {
                if (autoAddPrototype)
                {
                    Debug.Log("Prototype not found, adding it: " + prefab.name);

                    prototypeIndex = AddPrototype(terrainData, prefab);
                }
                else
                {
            ...
            if (localPosition.x >= 0 && localPosition.x <= 1 && localPosition.z >= 0 && localPosition.z <= 1)
            {
                ...
                float minDistance = (brushSize + overlapOffset) * 0.5f;
        ...
        /// <summary>
        /// Add a tree prefab to the terrain data as new prototype
        /// </summary>
        /// <param name="terrainData"></param>
        /// <param name="prefab"></param>
        /// <returns>index of the new prototype</returns>
        private int AddPrototype(TerrainData terrainData, GameObject prefab)
        {
            // Create a new tree prototype
            TreePrototype newTree = new TreePrototype();

            // Assign tree prefab
            newTree.prefab = prefab;

            // Additional optional settings, eg adjust bend factor
            // newTree.bendFactor = 0.5f;

            // Add to existing prototypes
            TreePrototype[] prototypes = terrainData.treePrototypes;

            System.Array.Resize(ref prototypes, prototypes.Length + 1);

            int newIndex = prototypes.Length - 1;
            prototypes[newIndex] = newTree;

            // Assign back to terrain data
        ...
        /// <summary>
        /// Remove all instances and prototypes from the terrain
        /// </summary>
        /// <param name="terrainData"></param>
        public void RemoveAllInstancesAndPrototypes()
        {
            TerrainData terrainData = GetTerrainData();
            ...
            Undo.RegisterCompleteObjectUndo(terrainData, "Remove all trees and prototypes");
